...
- **@docs/henshin/README.md**: Main index for Henshin knowledge base (reading order 00–09).
- **@docs/henshin/01-rule-anatomy.md**: Rule structure — LHS, RHS, mappings, actions.
- **@docs/henshin/02-parameters.md**: Parameter kinds (IN, OUT, INOUT, VAR) and MOMoT exposure.
- **@docs/henshin/07-common-patterns.md**: Complete XMI templates for common tasks.
- **@docs/henshin/09-debugging-runbook.md**: Triage steps when validation fails.

## Workflow

1. **Analyze Metamodel**: Read the target `.ecore` file to understand available classes, attributes, and references.
2. **Write Rule**: Create or modify the `.henshin` file. Use patterns from `docs/henshin/07-common-patterns.md` as starting templates.
...
5. **Apply Test**: If semantic validation passes, execute the rule against a small XMI model instance.
   ...
   ```
6. **Iterate**: On any error, consult `docs/henshin/09-debugging-runbook.md`, fix the rule, and return to step 3.
7. **MOMoT Integration**: Once locally verified, run a full smoke test via MCP `execute_momot_job`.

## Constraints

- Use project-relative paths for all file references (e.g. `tools/henshin-validator/validate.mjs`, `docs/henshin/examples/`).
