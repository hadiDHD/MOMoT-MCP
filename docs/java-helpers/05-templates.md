# Java-Helpers Wiki — Chapter 05: Three Canonical Java Helper Templates

This chapter provides three complete, copy-pasteable Java helper templates for implementing complex optimization scenarios in MOMoT.

---

## Template 1: Graph Metric BFS Path Length

Computes the maximum path length in a directed graph using Breadth-First Search (BFS).
...
public class GraphPathFitness extends AbstractEGraphFitness {
...
## Template 2: External Data Cost Matrix Lookup

Loads a static cost/distance matrix from an external file once, caching the results inside a static `HashMap` for fast O(1) lookups during evaluation.
...
public class CostMatrixFitness extends AbstractEGraphFitness {

    private static final Map<String, Double> costMatrix = new HashMap<>();

    static {
        // Static initializer loads the CSV cost matrix once during classloading
        try {
            InputStream is = CostMatrixFitness.class.getResourceAsStream("/cost_matrix.csv");
            if (is != null) {
    ...
    public double[] evaluate(TransformationSolution solution) {
        ...
        for (EObject assignment : assignments) {
            ...
            totalCost += costMatrix.getOrDefault(lookupKey, 999.0); // Default to a penalty cost
        }
...
## Template 3: Cached Incremental Fitness

Caches fitness values inside a thread-safe, weak-keyed map to avoid recalculating unchanged model graphs.
...
public class CachedFitness extends AbstractEGraphFitness {

    // Thread-safe weak-keyed map caches evaluations by graph instance reference
    ...
    public double[] evaluate(TransformationSolution solution) {
        ...
        // Return cached evaluation if graph state is unchanged
    ...
    private double runComplexStructuralAnalysis(EObject root) {
        // Complex, CPU-intensive analysis of containment tree
        try {
