...
async function setup({ forceDocker = false } = {}) {
  ...
  if (useDocker) {
    ...
    console.log('Compiling XmiValidator.java inside Docker...');
    const dockerArgs = [
      'run', '--rm',
      '-v', `${REPO_ROOT}:/workspace`,
      '-w', '/workspace',
      IMAGE,
      'javac', '-cp', 'tools/xmi-validator/lib/*', '-d', 'tools/xmi-validator/lib', 'tools/xmi-validator/src/XmiValidator.java'
    ];
    ...
    console.log('Compiling XmiValidator.java locally...');
    const classpath = join(LIB_DIR, '*');
    const javac = spawn('javac', ['-cp', classpath, '-d', LIB_DIR, join(SRC_DIR, 'XmiValidator.java')]);
    return new Promise((resolvePromise, reject) => {
      javac.on('close', (code) => {
        if (code === 0) resolvePromise();
...
async function run(args, { forceDocker = false } = {}) {
  ...
  if (useDocker) {
    ...
    const commandArgs = [
      ...
      '-w', '/workspace',
      IMAGE,
      'java', '-cp', 'tools/xmi-validator/lib/*:tools/xmi-validator/lib', 'XmiValidator', ...dockerArgs
    ];
    proc = spawn('docker', commandArgs);
  } else {
    const sep = process.platform === 'win32' ? ';' : ':';
    const classpath = join(LIB_DIR, '*') + sep + LIB_DIR;
    proc = spawn('java', ['-cp', classpath, 'XmiValidator', ...args]);
  }
  ...
  return new Promise((resolvePromise) => {
    proc.on('close', (code) => {
      if (stdout.trim()) {
        ...
        console.log(JSON.stringify({ success: false, errors: [`XmiValidator exited with code ${code}`], warnings: [], tier: 'structure' }));
      }
      resolvePromise(code);
    });
  });
...
if (args.includes('--setup')) {
  ...
  if (!existsSync(join(LIB_DIR, 'XmiValidator.class'))) {
    await setup({ forceDocker });
  }
